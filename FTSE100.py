...
# for full stock list on London Stock Exchange pages = range(1,1199) pagegs=[1,2] output_file_full =
# r'\\Galileo\Public\Legal Intelligence\Customer Segmentation\BA\Ad Hoc Reports & Requests\2019\201909 -
# September\DAI-2093 - Kenneth Ume - Market Product Penetration Data Request - REPORT\ftse_list.xlsx' lst_full = []

# for page in pages: url = r'https://www.londonstockexchange.com/exchange/prices-and-markets/stocks/prices-search
# /stock-prices-search.html?nameCode=&page={}'.format(page) response = requests.get(url) soup_full = BeautifulSoup(
# response.text, "html.parser") print(soup_full) table_full = soup_full.find('table', {'class': 'table_dati'}) print(
# table_full) table_rows_full = table_full.findAll('tr') print(table_rows_full) full_row = [] for tr_full in
# table_rows_full: td_full = tr_full.findAll('td') row_full = list(filter(lambda tr_full: tr_full != "",
# [tr_full.text.strip() for tr_full in td_full]))
